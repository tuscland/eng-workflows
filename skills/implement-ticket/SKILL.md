...
6. After the publication conditions pass, normally push the reviewed commit history and create or update the ticket's draft pull request under repository conventions. Explicit invocation authorizes this final publication. Target its declared pull-request base. Add the provider-native closing directive for the exact ticket—on GitHub, `Closes #<number>` in the same repository or `Closes <owner>/<repository>#<number>` across repositories—before the review record defined by the local loop contract. A bare URL, title mention, branch name, or comment is not a substitute.
7. Verify the remote head, pull-request base, and exact ticket association. Confirm that the remote body retained the closing directive. On GitHub, when the PR targets the default branch, inspect `closingIssuesReferences` and require the canonical repository and issue number. For a non-default integration base, verify the issue cross-reference and record that automatic closure is deferred to the final integration PR. Record and report the commit, PR, checks, cycles used, and local report path. Retain the worktree and run.
