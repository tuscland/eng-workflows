...
Apply **Decisions requiring human input** in the [shared loop contract](../../implement-ticket/references/local-review-loop.md). Investigate ordinary uncertainty about effort within the authorized scope, or defer an optional candidate with its rationale; an uncertain estimate alone does not require user guidance. When a proposal requires a consequential unresolved choice, present the concrete change, source evidence, expected benefit, effort/risk, alternatives, and recommendation. Request only that decision; previously accepted scope is already authorized. Continue independent selected work, but do not execute the dependent change or treat silence as approval. If the decision is required for completion, keep the run pending; optional work may be explicitly deferred.
