...
The delivery phase is local-only. Invocation authorizes local worktrees, ticket branches for multi-ticket delivery, direct implementation on the integration branch for a parent-only or single-ticket delivery, unsigned commits, local integration, verification, review, cleanup, end-to-end evidence, and the HTML delivery report. It does not authorize SSH, Git remote operations, commit signing, GitHub writes, pull-request creation, or publication. These local-only rules replace the delegated skills' tracker setup, publication, pull-request, remote-check, and merge steps for this invocation.
...
- For multiple required tickets, create each ticket on its declared local ticket branch from the current local integration tip. When the parent itself is the sole implementation unit, or the parent has exactly one required ticket, use the integration branch as that unit's implementation and ticket branch.
...
3. **Resolve the handoff.** Require a reachable local planning baseline and an integration branch distinct from the default and original target branches. For multiple required tickets, require a distinct declared branch for each ticket. For a parent-only or single-ticket delivery, require the sole implementation branch and ticket branch to equal the integration branch. Read the complete native child tree, blockers, acceptance criteria, and planning artifacts. Surface missing or conflicting metadata instead of inventing it.
...
5. **Implement the next ticket.** Process required tickets in deterministic topological order. A ticket is ready when every required blocker is reachable from the local integration tip or explicitly excluded by the specification. Invoke `$implement-ticket` with the ticket, parent handoff, and local-only contract. Let it complete its implementation and review/fix loop on the declared implementation branch, stopping before tracker setup or publication. A ticket clears only when its exact local head has a clean independent review, passing candidate checks, no unresolved finding or decision, and no signature.
6. **Gate and integrate locally.** Immediately before integration, require the implementation branch head to equal the reviewed candidate, the worktree to be clean, and the ticket diff to contain no unrelated commits. For a distinct ticket branch, also require the integration tip to equal its recorded expected base, then merge the ticket branch into the integration branch with a distinct unsigned merge commit that preserves the reviewed commits. When the implementation branch is the integration branch, require its reviewed candidate to be the integration tip and record direct-delivery evidence without creating a merge commit. Verify the reviewed ticket head and all blockers are ancestors of the resulting integration tip, then record the integration SHA and unsigned-commit evidence.
7. **Continue through the full tree.** Re-read the issue tree before selecting each ticket. Skip a ticket only when equivalent delivery is already reachable from the local integration branch with recorded review and integration evidence. Continue independent ready tickets when another ticket is blocked and their accepted bases and specifications remain valid.
...
9. **Verify the local handoff.** Require a clean integration worktree; every required ticket head and applicable merge commit reachable from the integration tip; no workflow-created commit containing a `gpgsig` header; passing required local checks and end-to-end scenarios; an accurate reviewed HTML report; and complete child and parent review records. Report the baseline limitation, delivered and excluded tickets, branch mapping and reviewed SHAs, local merge commits or direct-delivery evidence, integration head, checks, report path, review counts, decisions, and local run paths. Stop before signing or publication.
