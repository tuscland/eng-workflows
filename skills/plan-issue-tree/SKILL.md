...
1. Load the repository instructions and its issue-tracker, branch, worktree, and documentation conventions. Resolve the target branch and either the existing parent issue or the goal from which one will be created. Before creating a branch/worktree or publishing anything, locate and read `grill-with-docs`, `to-spec`, and `to-tickets`, and check that the configured tracker exposes the required issue and native relationship operations. Use read-only authentication/capability checks where available; do not make probe writes or add a permission checkpoint when authorization is already established. If a dependency is missing, report the exact prerequisite and continue independent read-only discovery; preserve existing work and resume after it is available. Do not substitute an improvised upstream workflow.
2. Before planning, establish the integration branch from the target branch in the current worktree. If the intended branch already exists, fetch and inspect it rather than creating a competing branch; ask the user when its ownership or state is ambiguous.
...
4. Validate, commit, and push only the documents produced by `/grill-with-docs`. If it produced no document changes, push the integration branch at its existing tip; no empty commit or artificial artifact is needed. The remote tip is the planning baseline. Establish this before either upstream skill publishes an issue as ready.
...
7. If tracker publication requires updates to the documents from step 4, validate, commit, and push only those document updates. Use the last pushed commit as the planning baseline. Record a visible handoff on the parent issue:
